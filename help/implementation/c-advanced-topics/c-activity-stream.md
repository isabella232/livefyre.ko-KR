---
description: Livefyre 시스템을 통해 사용자 생성 콘텐츠를 모니터링하고 저장하는 방법을 살펴봅니다.
seo-description: Livefyre 시스템을 통해 사용자 생성 콘텐츠를 모니터링하고 저장하는 방법을 살펴봅니다.
seo-title: 활동 스트림
...
Livefyre 시스템을 통해 사용자 생성 콘텐츠를 모니터링하고 저장하는 방법을 살펴봅니다.

Activity Stream API를 사용하여 네트워크 또는 사이트의 Livefyre 시스템을 통해 사용자 생성 데이터를 사용합니다. 예:이 API의 데이터를 사용하여 등급을 기준으로 검색 색인을 업데이트하거나 활동을 기반으로 타사 시스템에서 사용자 배지를 관리할 수 있습니다.
...
* **** 리소스: *문자열* 활동 데이터를 요청하는 개체의 URN입니다.

* **** 이후: *정수* 마지막 받은 이벤트의 ID를 나타내는 64비트 정수입니다. 이전 데이터가 없는 경우 '0'을 지정합니다.

## URN 문자열 {#section_skl_q4l_b1b}

예:

* **** urn:livefyre:에 `example.fyre.co` 대한 활동 스트림입니다 `example.fyre.co`.
* **** urn:livefyre:네트워크 `example.fyre.co:site=54321` 아래의 사이트 54321에 대한 활동 `example.fyre.co` 스트림입니다.

## 토큰 정책 {#section_nwh_c5j_11b}

활동 스트림 API는 인증에 OAuth 전달자 토큰을 사용합니다. 베어러 토큰은 OAuth 2.0 규격의 일부이며 [여기에서](https://tools.ietf.org/html/rfc6750#section-1.2)공식적으로 설명합니다.
...
* 발급자, 대상자, 대상, 만료 및 범위가 포함된 지도/사전을 만듭니다.
* 암호와 함께 JWT 라이브러리를 사용하여 JWT 토큰을 인코딩합니다.
* "인증 추가:HTTP 요청에 전달하십시오.

아래 코드 샘플에서는 Python의 위 단계를 보여 줍니다.
...
* **is** *(발급자)* 토큰을 생성할 권한이 있는 개체입니다. 사이트 또는 네트워크인 Livefyre일 수 있습니다. (학교에 지각하는 유서는 부모다.)
* **aud** *(대상자)* 이 토큰이 생성된 사람입니다. 직접 토큰을 만드는 경우 사이트 또는 네트워크입니다.
* **하위** ( *제목)* 권한을 부여할 제목입니다. 예를 들어 컬렉션에서 작업 중인 경우 제목은 컬렉션의 식별자여야 합니다. (학교 예제의 메모에서는, 당신입니다.)
* **exp** *(만료)* 토큰이 더 이상 유효하지 않은 시점입니다.
* **범위** *(범위)* 제목에 부여된 권한 목록입니다. '학교에 지각한다'는 말이 그 예다. API의 이름은 다른 예입니다.
...
* null이 아닌 경우 다음 요청의 `data.meta.cursor.next` 매개 변수로 `since` 값을 사용합니다.
* from 값이 null `data.meta.cursor.next` 이면 사용할 새 데이터가 없음을 의미합니다. 나중에 동일한 `since` 값으로 다시 요청하여 새 데이터가 도착했는지 확인해야 합니다.
* 실제로 값이 null이 아닌 경우 추가 데이터를 즉시 요청해야 `data.meta.cursor.next` 합니다.
* 프로덕션의 이 API를 통해 약 2시간 분량의 최근 데이터를 사용할 수 있습니다.
