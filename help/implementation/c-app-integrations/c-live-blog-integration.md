---
description: 라이브 블로그에서는 라이브 이벤트를 포함할 때 사이트의 편집자가 실시간으로 업데이트와 이미지를 사용할 수 있습니다.
seo-description: 라이브 블로그에서는 라이브 이벤트를 포함할 때 사이트의 편집자가 실시간으로 업데이트와 이미지를 사용할 수 있습니다.
seo-title: 라이브 블로그
...
라이브 블로그에서는 라이브 이벤트를 포함할 때 사이트의 편집자가 실시간으로 업데이트와 이미지를 사용할 수 있습니다.
...
라이브 블로그 앱을 내장하려면 앱 임베드에 대한 절차를 따르십시오. 앱 [포함을 참조하십시오](/help/implementation/c-livefyre-identity-comp/t-using-studio-to-connect-your-social-apps-to-your-livefyre-implementation.md). 다음은 임베드된 라이브 블로그 앱의 예입니다.
...
Collectionmeta는 인코딩된 JSON 개체입니다. 위의 예에서 JSON 개체는 JWT 인코딩하기 전에 다음 형식을 사용합니다.
...
| **네트워크** | *필수* 문자열livefyre가 제공하는 네트워크 이름입니다. 예를 들면 다음과 같습니다. *yourname. fyre. co.* |
| **Attachmentdelegate** | ** 선택적 개체 | 앱 스트림에 표시되는 미디어 첨부 파일 유형을 지정하는 데 사용됩니다. 자세한 내용은 미디어 [제한을](../c-app-customizations/c-restrict-media.md#c_restrict_media)참조하십시오. |
...
| **collectionmeta** | *필수* 문자열 | JWT 인코딩 메타데이터에 대한 메타데이터. 자세한 내용은 Collectionmeta 객체를 참조하십시오. |
...
| **Anonymousflaggingenabled** | Boolean | 손님 사용자에게 콘텐츠에 플래그를 지정할 수 있는 옵션이 있는지 여부를 정의합니다. 기본값은 true 입니다. |
...
| **checksum** | *선택적* 문자열 (권장) | Collectionmeta의 현재 상태를 식별합니다. 이 값을 변경하면 Livefyre가 서버의 데이터를 collectionmeta의 새 값으로 업데이트합니다. |
...
| **Editorcss** | ** 선택적 개체 | 주석 편집기 스타일링을 사용자 지정하는 데 사용됩니다. 편집기 내에 표시되는 텍스트의 글꼴 색상, 크기 및 글꼴은 물론 편집기의 필드 배경색을 스타일링할 수 있습니다. 예를 들면 다음과 같습니다. {background: ' # ccc ', 색상: ' red ', font: ' 30 px «Helvetica Neue», Helvetica, Arial, Geneva, sans-serif '} |
...
## 이벤트 핸들러 추가 {#concept_D835C710A7214F6D921571E0770B6BC5}
